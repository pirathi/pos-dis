			...
			<div style="overflow: hidden !important;display: flex; flex-wrap: wrap;align-content: center;width: {{$barcode_details->width * 1}}in; height: {{$barcode_details->height * 1}}in; justify-content: center;">
				...
				<div>
					...
					@if(!empty($print['business_name']))
						<b style="display: block !important; margin-top: 1px; font-size: {{$print['business_name_size']}}px">{{$business_name}}</b>
					@endif
					...
					@endif
					{{-- Barcode --}}
					<img style="max-width:90% !important;height: {{$barcode_details->height*0.28}}in !important; display: block;" src="data:image/png;base64,{{DNS1D::getBarcodePNG($page_product->sub_sku, $page_product->barcode_type, 1,24, array(0, 0, 0), false)}}">
					
					<span style="font-size: 13px; font-weight: bold; !important">
						{{strtoupper($page_product->sub_sku)}}
					</span>
				</div>
			</div>
